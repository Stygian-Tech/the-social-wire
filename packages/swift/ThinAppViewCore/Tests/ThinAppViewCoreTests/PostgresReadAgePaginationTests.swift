...
extension PostgresJetstreamInboxIntegrationTests {
  @Test("Larger read-age pages preserve exact legacy read-state snapshots and timestamp ties")
  func largerReadAgePages() async throws {
    try await PostgresInboxFixture.withFixture { fixture in
      ...
      try await execute("""
        ...
        """)
      // A floor covers the oldest URI at the timestamp tie; its unread override restores it.
      // An explicit read on the next URI remains excluded regardless of snapshot page size.
      try await execute("""
        INSERT INTO appview_publication_read_floors
          (viewer_did, publication_id, read_floor_at, read_floor_uri, generation, updated_at)
        VALUES (\(viewer), \(publication), \(timestamp), \(prefix + "0001"), 1, \(now))
        """)
      try await execute("""
        INSERT INTO appview_unread_overrides (viewer_did, subject_uri, created_at)
        VALUES (\(viewer), \(prefix + "0001"), \(now))
        """)
      try await execute("""
        INSERT INTO read_marks (viewer_did, subject_uri, created_at)
        VALUES (\(viewer), \(prefix + "0002"), \(now))
        """)
      ...
      #expect(larger.contains { $0.entryId == prefix + "0001" })
      #expect(!larger.contains { $0.entryId == prefix + "0002" })
      ...
      #expect(larger.first { $0.entryId == prefix + "0001" }?.publishedAt == now)
