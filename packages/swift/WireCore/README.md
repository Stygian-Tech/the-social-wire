...
4. Every minute by default, `wire-worker` prunes bounded graph state, refreshes community assignments when due, rebuilds exact rollups, loads eligible item/rollup/metadata rows, computes the deterministic `wire-v10` score, applies first-page diversity, and writes an immutable `wire_rank_generations` plus its `wire_ranked_items`. Baseline label snapshots remain throttled to five minutes, so faster generation does not increase labeler cadence. Metadata enrichment reserves three quarters of each multi-item batch for the most recently signaled eligible `und` stories whose page language has not been checked, while the remaining quarter continues the general refresh queue; this bounded priority lets exact-language generations recover without starving known-language metadata.
...
- it has at least five distinct high-intent actors in 24 hours or two explicit recommendations in 24 hours; a trusted Standard Site story with source confidence of at least `0.75` and published within 72 hours has a bounded lane at one distinct high-intent actor;
...
`wire-v10` applies an explainable commercial-content assessment before generation ranking.
...
The assessment is cumulative and reason-coded; it must not be an opaque keyword blocklist.
Ingestion and metadata enrichment derive bounded features once, retain the feature reasons
with the rebuildable item projection, and expose only an aggregate eligibility class or
penalty to the ranker. Do not repeatedly parse presentation strings during every generation.
Commercial quality is independent of `source_confidence`: an authentic, richly described
page may still be an advertisement, and source authenticity must remain separately
diagnosable.

Inspect every presentation source available for the canonical story:

- source-post text, embedded-card title/description, Standard Site title/summary/tags, and
  enriched OpenGraph title/description;
- the decoded canonical hostname, path components (including the slug), and semantic query
  keys retained by canonicalization;
- the final destination after the bounded metadata redirect policy, so a shortener or
  redirect cannot evade target classification;
- enriched OpenGraph title/description and a bounded boolean indicating JSON-LD
  `Product`, `Offer`, or nested `offers` schema. Raw page JSON is not retained.

Commercial feature reasons include, at minimum:

- `explicit_ad_disclosure` for sponsored, paid-partnership, `#ad`, `#sponsored`, or
  affiliate-link disclosure;
- `purchase_cta` for buy/shop/order language, promo codes, limited-time offers, trials,
  demo/registration lead generation, or follow/repost/tag giveaway mechanics;
- `price_or_discount`, `affiliate_parameter`, `referral_path`, `commercial_slug`,
  `product_offer_schema`, `tracking_parameters`, and `contact_solicitation`.

Tracking parameters alone are weak evidence because ordinary publishers use them. Honest
ad disclosure is strong classification evidence but is not, by itself, a moderation
violation. A starting calibration for offline evaluation is:

```text
explicit ad/affiliate disclosure       +4
Product/Offer structured metadata      +3
affiliate or referral parameter        +2
strong purchase or lead-generation CTA +2
price or discount language             +1
commercial path or slug                +1
tracking parameters only               +0.25
```
...
independent-discussion offsets remain reserved for a later revision; the active classifier
uses bounded per-item content, URL, topic, and metadata inputs. The retained `bsky.app`
domain penalty is compatibility configuration
only; it does not make a Bluesky post, profile, or feed destination eligible.
...
An `app.bsky.feed.post` is a signal envelope, never a Wire story. This includes direct
`at://.../app.bsky.feed.post/...` references, `bsky.app/profile/{actor}/post/{rkey}`
permalinks, embedded strong references to that collection, and redirects whose final
destination resolves to such a post. An ingested source post that carries an eligible
external card creates the article item and aliases that source-post AT URI to the article,
allowing later supported reference signals to resolve to the article. A direct Bluesky
permalink with no already-resolved article is acknowledged without creating an ineligible
item. Alternate-front-end recognition and bounded remote post/quote traversal remain future
work; any such implementation must collapse the chain to one canonical article.

The active ranked generation, quality reserve, simplified fallback, item-detail lookup,
and edition assembly must enforce the same target exclusion and commercial class. In
particular, degraded serving must not reintroduce probable ads or social-post destinations
that the primary ranker rejected. Target kind and bounded commercial reasons remain internal;
do not expose raw post text, commercial feature vectors, or internal scores through serving
DTOs.

To keep a sparse edition useful, `wire-v10` has a deterministic quality reserve. When the strict pool has fewer than 50 stories, candidates meeting the former three-high-intent-actor or one-recommendation floor may fill only the missing positions, but only when backed by usable Standard Site or OpenGraph presentation metadata. A fresh six-hour burst with at least three distinct high-intent shares in the last hour may enter before reaching the normal daily floor. Trusted direct Standard Site content within 72 hours and with source confidence of at least `0.75` may enter from its authoritative publication signal; this lane does not apply to social shares or generic OpenGraph links. Metadata and feedback never admit an article by themselves, burst and publication-lane stories still pass every quality, commercial, diversity, and moderation check, and reserve stories never displace strict stories. Locale-bucket discovery uses the same age, presentation, source-confidence, moderation, and signal lanes as the ranker, including the one-share fresh Standard Site lane.
...
Platform-hosted/social-media article destinations remain eligible but receive a bounded score subtraction because they are less likely to be the original publication. Direct Bluesky post, profile, and feed targets are the exception: target classification excludes them before ranking. Matching is exact-host or dot-suffix only, so `news.youtube.com` matches while `notyoutube.com` does not. Defaults are `0.06` for `youtube.com`, `youtu.be`, and `twitch.tv`; `0.04` for `reddit.com` and `redd.it`; and `0.05` for `bsky.app`, `facebook.com`, `fb.com`, `fb.watch`, `instagram.com`, `linkedin.com`, `pinterest.com`, `threads.net`, `tiktok.com`, `twitter.com`, and `x.com`. The retained `bsky.app` penalty is compatibility protection for previously stored candidates that predate target classification. The longest matching suffix wins. Each configured penalty must be finite and in `[0, 0.20]`. Penalties affect ordering only for eligible targets: they never change admission tier or weaken moderation. A strong independently qualified article can still rank.
...
`wire-v10` remains the sole authority for story eligibility and order. `wire-edition-v2`
...
1. Select up to four lead stories—one feature and three supporting stories—accepting only
   ...
   tie-break; it does not alter `wire-v10` scores, eligibility, or continuation order.
...
or fetch metadata per card. An effective HTTP(S) presentation thumbnail is tracked separately
from the broad OpenGraph availability bit: bounded candidate loading prefers image-bearing
stories, and the ranker applies only the documented missing-thumbnail penalty when artwork is
absent. `wire_link_metadata_cache`
...
`und` is the multilingual global bucket used when a request resolves to no specific language. Every global worker cycle builds it first, then discovers and builds at most 12 observed locale buckets. Global and language-specific generations require at least 50 eligible candidates, exactly the complete diverse first-page target. Language tags are lowercase BCP 47 primary languages, maximum 35 characters. Catalog languages are capped at 12. Article metadata enrichment treats the page's normalized HTML `lang`, OpenGraph locale, or Schema.org `inLanguage` as stronger evidence than the language declared by a social post sharing that article. The metadata cache records when this language evaluation completed; older successful cache rows are re-fetched without conditional validators in bounded worker batches so a legacy ETag cannot preserve a weak share-post classification indefinitely.
...
- never substitute the multilingual `und` generation for an explicit locale request;
...
- when no retained generation exists, serve a deterministic engagement-gated quality fallback restricted to the exact requested language; the fallback prefers Standard Site stories and requires trusted provenance, usable OpenGraph metadata, or high source confidence, and it never synthesizes or merges a client-side feed.
...
`WireCursorCodec` accepts at least 32 bytes of secret material. Its version-1 payload contains exactly the generation ID, language, and next ordinal; the token is `base64url(payload).base64url(HMAC-SHA256(payload))`. Decode rejects malformed, oversized (>4,096 bytes), invalid-signature, unsupported-version, negative-ordinal, empty/oversized-generation, and empty/oversized-language tokens. Serving rejects a cursor whose language differs from the request as `InvalidRequest`; a correctly scoped cursor whose retained generation has expired returns `CursorExpired`. Cursor HMAC secrets are server-only and rotate with an overlap window.

## Moderation, privacy, and presentation

Moderation is fail-closed. Before generation, the worker queries each configured ATProto labeler for exact candidate representative AT-URIs and public source-author DIDs in bounded batches, including the trusted one-share Standard Site admission lane. A fully paginated snapshot atomically projects network takedown, `!hide`, spam, adult/sexual, and graphic-media labels; negations, expiry, and absence remove prior projections. The launch-time local base content classifier projects only high-confidence explicit page evidence into the same `wire_labels` gate under `app.thesocialwire.base-content-labeler`; broad contextual words alone remain insufficient. The worker excludes active block/exclude/adult/graphic/spam labels, and the serving adapter repeats that check so an already-generated item can be removed immediately. A failed, incomplete, or stale baseline refresh prevents generation activation and preserves the last good label rows. If viewer moderation state is unavailable, return `ModerationUnavailable`, not an unfiltered edition.
