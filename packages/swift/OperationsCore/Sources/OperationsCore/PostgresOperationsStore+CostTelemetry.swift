...
extension PostgresOperationsStore {
  /// Each schedule owns its throttle, query failures and export. No cached value is re-emitted
  /// with a newer timestamp; unavailable observations are omitted rather than reported as zero.
  public func recordDatabaseCostTelemetry(group: DatabaseCostTelemetryGroup, at: Date) async {
    guard !Task.isCancelled else { return }
    guard !runningDatabaseCostObservations.contains(group) else { return }
    if let last = lastDatabaseCostObservations[group],
      at.timeIntervalSince(last) < group.intervalSeconds { return }
    runningDatabaseCostObservations.insert(group)
    defer { runningDatabaseCostObservations.remove(group) }
    lastDatabaseCostObservations[group] = at
    ...
    switch group {
    case .counters:
      do { try await recordDatabaseObservabilityCounters(at: at) } catch {
        logger.debug("Database overview counters unavailable")
      }
      guard !Task.isCancelled else { return }
      do {
    ...
    case .tables:
      do { try await recordDatabaseObservabilityTables(at: at) } catch {
        logger.debug("Database overview table sizes unavailable")
      }
      guard !Task.isCancelled else { return }
      do {
        guard !Task.isCancelled else { return }
        ...
        logger.debug("Table cost counters unavailable")
      }
    case .expiry:
      let tables = DatabaseExpiryTelemetryTable.allCases
      let table = tables[nextDatabaseExpiryTableIndex]
      // An unavailable table must not pin the rotation and starve the other nine.
      nextDatabaseExpiryTableIndex = (nextDatabaseExpiryTableIndex + 1) % tables.count
      do {
        for row in try await databaseExpiryBacklogRows(table: table, at: at) {
