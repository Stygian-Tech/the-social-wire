...
Use the round config's explicit `memory_limit_bytes` unchanged in the runner and database probe: `13000000000`, `12000000000`, `11000000000`, then `10000000000` for Railway's decimal 13, 12, 11 and 10 GB trials (`16000000000` remains the baseline). Preflight and every observed sample must match the exact 4096-byte page floor of that requested cap, using `page_size_bytes` measured by the probe inside the target container. The runner never uses its own host page size, applies a tolerance, translates GB to GiB, or accepts the retired `memory_gib` field. Summaries retain requested `memory_limit_bytes`, raw cgroup `memory_max`, and measured `page_size_bytes`. Legacy exact 16 GB/8 GB observations without page metadata remain explicitly supported; lower-cap evidence must supply the measured page size. See the probe guide for exact values.
...
| `replay` | Supervised long-lived intake of a reviewed fixed archive range through existing replay facilities into the isolated target. Disable live upstream collection and restored source-worker credentials. Emit JSONL `{"completed":<cumulative uniquely acknowledged events>,"snapshot_complete":<boolean>,"drain_complete":<boolean>}` from committed database receipts. The last two fields independently prove exact sealed archive bounds and no unresolved source work. Mere archive submissions are not successful throughput. Use `trial_replay_adapter.py` with the pinned real Go/Swift binaries and reviewed manifest described in `trial-replay-adapter.md`; it installs trial-only receipts before starting its source. No hosted capacity run is implied by the adapter. |
