...
export function LoginHandleTypeahead({
  ...
  const chooseSuggestion = (handle: string) => {
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      event.preventDefault();
      chooseSuggestion(suggestions[activeIndex].handle);
    ...
    <div className="relative">
      ...
      {open ? (
        <div
          id={listboxId}
          role="listbox"
          aria-label="Handle Suggestions"
          className="absolute inset-x-0 top-[calc(100%+0.375rem)] z-50 overflow-hidden rounded-xl border bg-popover p-1 shadow-lg"
        >
          {suggestions.map((suggestion, index) => (
            <button
              ...
              onClick={() => chooseSuggestion(suggestion.handle)}
            >
              <Avatar
