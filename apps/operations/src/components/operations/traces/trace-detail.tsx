    ...
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-3">
      <OperationsSection title={<span className="font-mono">Trace {traceId} · Recorded Spans ({spans.length})</span>}>
        <div className="grid gap-2 p-3 md:hidden">
          {spans.map((span) => (
            <article key={span.id} className="ops-record-card">
          ...
          {spans.map((span) => (
            <section key={span.id} className="p-3">
              ...
              {Object.keys(span.attributes).length ? (
                <dl className="ops-data-grid mt-2 grid gap-px text-[10px] sm:grid-cols-2">
                  {Object.entries(span.attributes).map(([key, value]) => (
                    <div key={key} className="ops-data-cell grid grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)] gap-2">
