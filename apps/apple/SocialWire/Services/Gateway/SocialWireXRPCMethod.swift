enum SocialWireXRPCMethod {
    private static let prefix = "/xrpc/"

    static let getPreferences = prefix + "app.thesocialwire.sync.getPreferences"
    static let getSidebar = prefix + "app.thesocialwire.publication.getSidebar"
    static let refreshSidebar = prefix + "app.thesocialwire.publication.refreshSidebar"
    static let resolvePublication = prefix + "app.thesocialwire.publication.resolvePublication"
    static let getFeed = prefix + "app.thesocialwire.appview.getFeed"
    static let listEntries = prefix + "app.thesocialwire.appview.listEntries"
    static let getEntry = prefix + "app.thesocialwire.appview.getEntry"
    static let getUnreadCounts = prefix + "app.thesocialwire.appview.getUnreadCounts"
    static let getReadStateStatus = prefix + "app.thesocialwire.appview.getReadStateStatus"
    static let exportReadState = prefix + "app.thesocialwire.appview.exportReadState"
    static let prepareReadState = prefix + "app.thesocialwire.appview.prepareReadState"
    static let confirmReadState = prefix + "app.thesocialwire.appview.confirmReadState"
    static let putReadMark = prefix + "app.thesocialwire.appview.putReadMark"
    static let deleteReadMark = prefix + "app.thesocialwire.appview.deleteReadMark"
    static let markAllRead = prefix + "app.thesocialwire.appview.markAllRead"
    static let getReadAgeOptions = prefix + "app.thesocialwire.appview.getReadAgeOptions"
    static let markReadBefore = prefix + "app.thesocialwire.appview.markReadBefore"
    static let enrollSources = prefix + "app.thesocialwire.appview.enrollSources"
    static let purgeViewerData = prefix + "app.thesocialwire.appview.purgeViewerData"
    static let getWire = prefix + "app.thesocialwire.discovery.getWire"
    static let getWireEdition = prefix + "app.thesocialwire.discovery.getWireEdition"
    static let getWireItem = prefix + "app.thesocialwire.discovery.getWireItem"
    static let getFeedCatalog = prefix + "app.thesocialwire.discovery.getFeedCatalog"
    static let getCircleCatalog = prefix + "app.thesocialwire.discovery.getCircleCatalog"
    static let getCircleEdition = prefix + "app.thesocialwire.discovery.getCircleEdition"
    static let setCircleItemHidden = prefix + "app.thesocialwire.discovery.setCircleItemHidden"
}
