...
The edge queries only `wire_serving` views. Those views apply corpus eligibility, retention, baseline-label exclusions, and generation lifecycle while omitting raw signals, actor hashes, scores, diagnostics, counts, and viewer/Operations state. The edge response contains presentation fields, rank ordinal, reasons, provenance, and the public source/author identifier needed for local viewer filtering.
...
AppView sends no viewer DID, access token, DPoP proof, cookie, muted word, or block/mute set to Production. It validates the edge contract version, applies viewer moderation from the viewer's own PDS, and mints a cursor with the environment-local `WIRE_CURSOR_HMAC_SECRET`. A corpus outage returns The Wire unavailable and does not change AppView readiness or the local Subscribed/Following path.
