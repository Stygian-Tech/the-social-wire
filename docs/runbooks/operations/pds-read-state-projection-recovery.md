...
Changing the collection filter under an existing generation is rejected by immutable identity validation. This procedure is an unexecuted operator gate; the audit did not establish the current lease holder or record a handoff seam. The three public read-state Lexicons were published and their complete records/CIDs verified on September 9; publication does not itself activate ingestion or clients.
