  ...
  func authoritativeValidation() async throws {
    ...
    do {
      ...
      for elapsed in [0.0, 120.0] {
        ...
        #expect(try await cached.edition(language: "en", region: nil, now: observedAt).edition.leadStories.count == 1)
      ...
      let originalActor = "did:plc:original"
      let changedActor = "did:plc:changed"
      try await pool.query("UPDATE wire_items SET author_key = \(originalActor) WHERE canonical_key = \(key)", logger: logger)
      #expect(try await cached.edition(language: "en", region: nil, now: now).sourceActorKeysByItemID?[key] == originalActor)
      try await pool.query("UPDATE wire_items SET author_key = \(changedActor) WHERE canonical_key = \(key)", logger: logger)
      #expect(try await cached.edition(language: "en", region: nil, now: now).sourceActorKeysByItemID?[key] == changedActor)
      ...
      #expect(try await cached.edition(language: "en", region: nil, now: now).edition.leadStories.first?.title == "After")
      try await pool.query("""
      ...
      #expect(try await cached.edition(language: "en", region: nil, now: now).edition.leadStories.isEmpty)
      try await pool.query("DELETE FROM wire_labels WHERE canonical_key = \(key)", logger: logger)
      #expect(try await cached.edition(language: "en", region: nil, now: now).edition.leadStories.count == 1)
