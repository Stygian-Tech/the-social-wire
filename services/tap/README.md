...
the environment-specific Railway private hostname.
...
Required Railway variables, set independently in each environment:
...
- `TAP_BASE_URL=http://${{Tap.RAILWAY_PRIVATE_DOMAIN}}:${{Tap.PORT}}` on Charybdis.
...
Because the admin password protects every HTTP route, Tap has no unauthenticated HTTP health check.
Readiness and repository-sync health come from the authenticated Tap capability and worker evidence.
