...
actor ATProtoAuthenticatedRepoClient {
  ...
  struct RepoRecord: Sendable {
    let uri: String
    let cid: String?
    let value: PdsRecordJSON
  }
  ...
  func getRecordByAtUri(auth: AuthContext?, atUri: String) async throws -> PdsRecordJSON? {
    guard let parsed = RenderFieldExtractor.parseAtUri(PublicationProjectionLogic.normalizeAtRepoParam(atUri)) else {
      return nil
    }
