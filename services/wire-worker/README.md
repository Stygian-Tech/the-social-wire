...
| `WIRE_INBOX_BATCH_SIZE` | `1000` | Maximum passive-reference fast-path rows per drain iteration; ordinary claims use the smaller of this value and concurrency (maximum `5000`) |
...
Before any non-off generation is built, the worker queries every configured labeler through `com.atproto.label.queryLabels`. It includes the one-share trusted Standard Site lane, sends exact candidate representative AT-URIs and public source-author DIDs in batches of at most 25, requests at most 250 labels per page, follows at most 20 advancing cursors per batch, and runs at most four batch requests concurrently. Each HTTP request has a five-second timeout and the complete baseline refresh has a 45-second deadline. The default is Bluesky's moderation service (`did:plc:ar7c4by46qjdydhdevvrndac|https://mod.bsky.app`); operators can replace or extend it with `WIRE_BASELINE_LABELERS`. A conservative local content classifier also projects high-confidence explicit external pages into `wire_labels` under `app.thesocialwire.base-content-labeler`; it complements rather than replaces the configured network labelers.
...
The isolated Go lane stages global events into `wire_ingestion_inbox`. A dedicated runtime continuously claims bounded batches using PostgreSQL fencing and `SKIP LOCKED`. Before an ordinary claim, one atomic worker-mediated fast path applies at most `WIRE_INBOX_BATCH_SIZE` ready repository-head `create`/`update` likes and reposts whose payload has a non-empty subject URI and whose subject has no live `wire_item_aliases` row. The selection locks rows with `FOR UPDATE SKIP LOCKED` and the status/readiness predicates are fenced again by the update, so peer workers cannot consume the same row. It does not lease rows into memory and does not bypass a prior pending, leased, or retry event in the same repository. A source-scoped drain may also lease a bounded contiguous repository prefix of like/repost deletes together; those retractions commute by record URI, while any earlier non-passive event remains a hard FIFO barrier. Malformed events, live-alias references, future retries, and all high-intent Standard Site recommendations and Wire feedback stay on the ordinary path. Fast-path rows use the normal five-minute applied retention and remain counted by admission until terminal cleanup deletes them and decrements the counter transactionally.

Each ordinary claim is capped at `min(WIRE_INBOX_BATCH_SIZE, WIRE_INBOX_CONCURRENCY)` so one replica cannot lease thousands of repository heads into a process-local iterator and starve peer drainers. A batch contains only the earliest actionable event for each repository, so up to `WIRE_INBOX_CONCURRENCY` different repositories can apply concurrently without violating per-repository sequence. Ready work is ordered by `next_attempt_at` before sequence, preventing older retry rows in unrelated repositories from monopolizing a bounded claim. Full batches continue immediately; an empty iteration waits `WIRE_INBOX_IDLE_MILLISECONDS`. Failures preserve leases/retry state, make readiness unhealthy, and retry with bounded backoff. Unresolved high-intent Standard Site recommendations and Wire feedback retry for at most 24 hours before dead-lettering, so a temporarily reordered public dependency can still resolve. Unresolved Bluesky likes and reposts are applied no-ops because passive signals cannot admit a story and must not block a repository FIFO. One process therefore performs at most one bounded database fast path plus one concurrency-bounded ordinary claim per iteration and cannot create an unbounded in-memory queue.
...
count and per-second rate for that replica, plus the global ready/expired-lease backlog
count and oldest staged age. It never includes a DID, repository, actor hash, URL, or
story key. The backlog query uses the ready and expired-lease indexes inside a five-second
statement timeout and runs independently of the drain loop. A telemetry query failure is
reported without changing readiness or interrupting drain work, and the applied count is
retained for the next successful interval.
